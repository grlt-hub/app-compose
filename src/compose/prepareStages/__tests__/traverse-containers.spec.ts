...
describe('traverseContainers', () => {
  ...
  test('should return an empty list if an empty array of containers is passed', () => {
    ...
    expect(result.strictDependencies).toEqual(new Set([]));
  });
  ...
  test('should resolve only strict dependencies by default', () => {
    ...
    expect(Array.from(result.strictDependencies)).toEqual(expect.arrayContaining([containerA, containerB, containerC]));
    expect(result.strictDependencies.size).toBe(3);
  });
  ...
  test('should handle cyclic dependencies without getting stuck in a loop', () => {
    ...
    expect(Array.from(result.strictDependencies)).toEqual(
      expect.arrayContaining([containerA, containerB, containerC, containerD]),
    );
    expect(result.strictDependencies).toHaveLength(4);
  });
});
