...
describe("when.every", () => {
  it("returns true when predicate holds for all tasks", async () => {
    ...
    const result = when.every([login, logout], (s) => s.result.id < 5)
  ...
  it("returns true when predicate holds for all tags", async () => {
    ...
    const result = when.every([userId, sessionId], (x) => x > 0)
  ...
  it("supports mixed tags and tasks in the list", async () => {
    ...
    const result = when.every([userId, login], (s) => (typeof s === "number" ? s > 0 : s.result.id < 10))
  ...
  it("supports Spot in the list alongside tasks", async () => {
    ...
    const result = when.every([login, optional(logout.result)], (x) => {
      if (!x) return false
...
describe("when.every.status", () => {
  it("returns true when all tasks reach the given status", async () => {
    ...
    const result = when.every.status([login, logout], "done")
  ...
  it("returns false when one task does not match the status", async () => {
    ...
    const result = when.every.status([login, broken], "done")
  ...
  it("matches against the given status (fail)", async () => {
    ...
    const result = when.every.status([broken1, broken2], "fail")
  ...
  it("matches against the given status (skip)", async () => {
    ...
    const result = when.every.status([skipped1, skipped2], "skip")
  ...
  it("supports optional status spots in the list", async () => {
    ...
    const result = when.every.status([optional(login.status), optional(logout.status)], "done")
