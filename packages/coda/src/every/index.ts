...
const every = <const List extends readonly (Task<unknown> | Tag<unknown> | Spot<unknown>)[]>(
  list: List,
  ...
  shape(
    ...
    (values) => values.every(predicate as (v: any) => boolean),
  )

every.status = <const Items extends readonly (Task<unknown> | Spot<TaskStatus | undefined>)[]>(
  items: Items,
  status: TaskStatus,
): Spot<boolean> =>
  shape(
    items.map((item) => (is.task(item) ? item.status : item)),
    (statuses) => statuses.every((s) => s === status),
  )
