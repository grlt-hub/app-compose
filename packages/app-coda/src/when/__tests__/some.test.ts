...
describe("when.some", () => {
  it("returns true when predicate holds for at least one task", async () => {
    ...
    const result = when.some([login, logout], (s) => s.result.id < 5)
  ...
  it("returns true when predicate holds for at least one tag", async () => {
    ...
    const result = when.some([userId, sessionId], (x) => x > 0)
  ...
  it("supports mixed tags and tasks in the list", async () => {
    ...
    const result = when.some([userId, login], (s) => (typeof s === "number" ? s > 0 : s.result.id < 10))
  ...
  it("supports Spot in the list alongside tasks", async () => {
    ...
    const result = when.some([login, optional(logout.result)], (x) => {
      if (!x) return false
...
describe("when.some.status", () => {
  it("returns true when at least one task reaches the given status", async () => {
    ...
    const result = when.some.status([login, broken], "done")
  ...
  it("returns false when no task matches the status", async () => {
    ...
    const result = when.some.status([broken1, broken2], "done")
  ...
  it("matches against the given status (fail)", async () => {
    ...
    const result = when.some.status([login, broken], "fail")
  ...
  it("matches against the given status (skip)", async () => {
    ...
    const result = when.some.status([login, skipped], "skip")
  ...
  it("supports optional status spots in the list", async () => {
    ...
    const result = when.some.status([optional(login.status), optional(logout.status)], "done")
