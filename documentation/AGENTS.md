...
This file extends the repository-level `AGENTS.md` for everything under `documentation/`.
...
- `Scope`, `Context`, and `Spot` are codebase terms — never use them as casual nouns in prose; it creates a false anchor to the term.
...
Use package source and tests as the source of truth for API behavior. For site-wide verification, run
`pnpm doc:build` from the repository root.
