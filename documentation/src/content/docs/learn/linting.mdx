---
title: Linting
description: Enforce App-Compose conventions with the ESLint plugin.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

An ESLint plugin enforces App-Compose conventions in TypeScript code. It requires ESLint 9+.

## Installation

Use your preferred package manager.

<Tabs>
  <TabItem label="npm">

    ```bash
    npm install --save-dev --save-exact @app-compose/eslint-plugin
    ```

  </TabItem>
  <TabItem label="pnpm">

    ```bash
    pnpm add --save-dev --save-exact @app-compose/eslint-plugin
    ```

  </TabItem>
  <TabItem label="yarn">
   
    ```bash
    yarn add --dev --exact @app-compose/eslint-plugin
    ```

  </TabItem>
  <TabItem label="bun">

    ```bash
    bun add --dev --exact @app-compose/eslint-plugin
    ```

  </TabItem>
</Tabs>

## Usage

Import the plugin and add it to your config. Use the recommended preset for a quick start.

{/* oxfmt-ignore */}

```js
import appCompose from "@app-compose/eslint-plugin"
import tseslint from "typescript-eslint"

export default tseslint.config(appCompose.configs.recommended)
```

You can also configure it manually.

```js
{
  plugins: {
    'app-compose': appCompose,
  },
  rules: {
    'app-compose/task-options-order': 'warn',
  },
}
```

## Rules

### no-async-shape-callback

❗ This rule errors in the `recommended` config.

💭 This rule requires [typed linting](https://typescript-eslint.io/getting-started/typed-linting/).

<hr />

In a `shape()` callback, async work runs outside the chain, and App-Compose cannot order it. The plugin reports
every callback that returns a `Promise`.

```ts
// ❌ Wrong
shape(user.result, async (u) => await fetchAvatar(u.id))

// ✅ Correct
shape(fetchAvatar.result, (a) => a.url)
```

### no-coda-debug

❗ This rule errors in the `recommended` config.

<hr />

The `debug()` helper from `@app-compose/coda` prints task state to the console. The plugin reports every call to
it, so it stays out of committed code.

```ts
import { compose } from "@app-compose/core"
import { debug } from "@app-compose/coda"

// ❌ Wrong
compose().step(login).step(debug(login)).run()

// ✅ Correct
compose().step(login).run()
```

### no-scope-get

This rule is not enabled in the `recommended` config. Enable it explicitly when `scope.get()` should be treated as
debug-only code.

💭 This rule requires [typed linting](https://typescript-eslint.io/getting-started/typed-linting/).

```js
{
  rules: {
    'app-compose/no-scope-get': 'warn',
  },
}
```

<hr />

The plugin reports every call to `scope.get()` on an App-Compose Scope, including Scopes returned by `run()` and
Scopes received by observers. Other objects with a method named `get` are not reported.

```ts
// ❌ Wrong
const scope = await compose().step(login).run()
scope.get(login.result)

// ✅ Correct
await compose().step(login).run()
```

### task-options-order

⚠️ This rule warns in the `recommended` config.

🔧 This rule is automatically fixable by the [`--fix` CLI option](https://eslint.org/docs/latest/user-guide/command-line-interface#--fix).

<hr />

This order matches how a Task reads top-to-bottom: `name` first, then `run` before the optional `enabled`. Inside both,
`context` comes before `fn`. The plugin reports when the order differs.

```ts
// ❌ Wrong
createTask({
  name: "alpha",
  enabled: {
    fn: check,
    context: { authorized: authorizedTag },
  },
  run: {
    context: { timeout: timeoutTag },
    fn: init,
  },
})

// ✅ Correct
createTask({
  name: "alpha",
  run: {
    context: { timeout: timeoutTag },
    fn: init,
  },
  enabled: {
    context: { authorized: authorizedTag },
    fn: check,
  },
})
```

### wire-options-order

⚠️ This rule warns in the `recommended` config.

🔧 This rule is automatically fixable by the [`--fix` CLI option](https://eslint.org/docs/latest/user-guide/command-line-interface#--fix).

<hr />

A Wire carries data from a source to a destination tag, so `from` is listed before `to`. The plugin reports when
the order differs.

```ts
// ❌ Wrong
createWire({
  to: apiUrl,
  from: literal("https://api.example.com"),
})

// ✅ Correct
createWire({
  from: literal("https://api.example.com"),
  to: apiUrl,
})
```
