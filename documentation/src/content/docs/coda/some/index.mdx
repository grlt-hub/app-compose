...
```ts
// without some
const anyEven = shape([fetchUser.result, fetchOrder.result], (results) => results.some((result) => result.id % 2 === 0))
...
```ts
// without some
const anyDone = shape([fetchUser.status, fetchOrder.status], (statuses) => statuses.some((status) => status === "done"))
