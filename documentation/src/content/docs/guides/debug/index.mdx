...
[debug](/coda/debug) from `@app-compose/coda` handles this wrapping for you. Pass any Tasks, Tags, or Spots; it returns a Task you can `.step(...)`. A lint rule [no-coda-debug](/learn/linting/#no-coda-debug) flags every call so it doesn't reach a commit.
